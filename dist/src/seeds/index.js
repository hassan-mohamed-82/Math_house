...
async function main() {
    try {
        ...
        console.log("\n📏 Seeding Grades...");
        const { seedGrades } = await Promise.resolve().then(() => __importStar(require("./grade")));
        ...
        console.log("\n💰 Seeding Prices...");
        const { seedPrices } = await Promise.resolve().then(() => __importStar(require("./prices")));
        await seedPrices();
        // 8. Students (depends on categories, grades)
        console.log("\n🎓 Seeding Students...");
        await (0, students_1.seedStudents)(categoryMap, gradeMap);
        // 9. Parents
        console.log("\n👪 Seeding Parents...");
