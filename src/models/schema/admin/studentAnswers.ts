...
export const studentAnswers = mysqlTable("student_answers", {
    ...
    attemptId: char("attempt_id", { length: 255 }).notNull().references(() => examAttempts.id),
    questionId: char("question_id", { length: 255 }).notNull().references(() => questions.id),
    selectedOptionId: char("selected_option_id", { length: 255 }).references(() => questionOptions.id),
